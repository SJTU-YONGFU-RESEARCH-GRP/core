...
def get_tool_versions() -> Dict[str, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            verilator_version = clean_tool_version(result.stdout)
    ...
    try:
        result = subprocess.run(["iverilog", "-V"], 
                              ...
                              check=False)
        if result.returncode == 0:
            iverilog_version = clean_tool_version(result.stdout)
    ...
    try:
        result = subprocess.run(["gtkwave", "--version"], 
                              ...
                              check=False)
        if result.returncode == 0:
            gtkwave_version = clean_tool_version(result.stdout)
...
def generate_report(results, module_mappings):
    ...
    with open(REPORT_FILE, "w") as f:
        ...
        for i, category in enumerate(sorted(categories.keys()), 3):
            ...
            f.write(f"{i}. [{category}](#{category_id})\n")
        ...
        for category, targets in sorted(categories.items()):
            ...
            category_id = category.lower().replace(' ', '-').replace('/', '-')
            f.write(f"| [{category}](#{category_id}) | {cat_total} | {cat_passed} | {cat_failed} | {pass_rate:.1f}% | {cat_tests_passed}/{cat_tests_run} | {cat_avg_runtime:.2f} |\n")
        
        # Detailed results by category
        f.write("\n## Detailed Results\n\n")
        
        for category, targets in sorted(categories.items()):
            # Use standard markdown header which creates automatic anchors
            f.write(f"## {category}\n\n")
