...
"""
    ...
    def generate_verification_report(self):
        """Generate a verification report with proper formatting for tool versions and links"""
        # This method would parse existing REPORT.md and fix formatting issues
        report_path = 'REPORT.md'
        if not os.path.exists(report_path):
            self.logger.warning(f"REPORT.md not found at {report_path}")
            return ""
        
        with open(report_path, 'r') as f:
            content = f.read()
        
        # Fix tool version table
        content = self.fix_tool_version_table(content)
        
        # Fix hyperlinks
        content = self.fix_hyperlinks(content)
        
        return content
    
    def fix_tool_version_table(self, content):
        """Fix the tool version table formatting"""
        # Find the tool version table section
        tool_section_start = content.find('## Tool Versions')
        if tool_section_start == -1:
            return content
        
        tool_section_end = content.find('\n## ', tool_section_start + 1)
        if tool_section_end == -1:
            tool_section_end = len(content)
        
        # Extract current tool section
        tool_section = content[tool_section_start:tool_section_end]
        
        # Create a new properly formatted tool table
        new_tool_section = """## Tool Versions
...
"""
        ...
        new_content = content[:tool_section_start] + new_tool_section + content[tool_section_end:]
        return new_content
    
    def fix_hyperlinks(self, content):
        """Fix hyperlinks in the table of contents and category references"""
        # Fix table of contents links
        lines = content.split('\n')
        fixed_lines = []
        
        for line in lines:
            # Fix TOC links like "5. [Encoding/Decoding](#category-encoding/decoding)"
            if re.match(r'^\d+\.\s+\[.*\]\(#category-.*\)', line):
                # Extract the category name
                match = re.search(r'\[(.*?)\]', line)
                if match:
                    category_name = match.group(1)
                    anchor_id = create_anchor_id(f"category-{category_name}")
                    # Replace the link
                    line = re.sub(r'\(#category-.*?\)', f'(#{anchor_id})', line)
            
            # Fix category headers like "<a name='category-encoding/decoding'></a>"
            if '<a name=' in line and 'category-' in line:
                match = re.search(r"<a name='category-(.*?)'></a>", line)
                if match:
                    category_name = match.group(1)
                    anchor_id = create_anchor_id(f"category-{category_name}")
                    line = f"<a name='{anchor_id}'></a>"
            
            # Fix "Back to top" links
            if '[Back to top](#table-of-contents)' in line:
                line = line.replace('[Back to top](#table-of-contents)', '[Back to top](#table-of-contents)')
...
def clean_tool_version(version_text):
    """Clean tool version output for proper table formatting"""
    if not version_text:
        return "Unknown"
    ...
    for line in lines:
        line = line.strip()
        if line and not line.startswith('Copyright') and not line.startswith('This is'):
...
def create_anchor_id(text):
    """Create a proper markdown anchor ID from text"""
...
def main():
    parser = argparse.ArgumentParser(description="Analyze Verilog dataset and generate DATASET.md or REPORT.md")
    ...
    parser.add_argument('--report', action='store_true', help='Generate verification report format instead of dataset analysis')
    ...
    if args.report:
        # Fix existing REPORT.md formatting issues
        md_content = analyzer.generate_verification_report()
        if not md_content:
            logger.error("Failed to process REPORT.md")
            return
    else:
        # Generate regular dataset analysis
        analyzer.analyze_directory()
        md_content = analyzer.generate_markdown_report()
        
    with open(args.output, 'w') as f:
        f.write(md_content)
    logger.info(f"Report processing complete! Fixed report saved to {args.output}")
