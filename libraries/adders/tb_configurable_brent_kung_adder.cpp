#include "Vconfigurable_brent_kung_adder.h"
#include "verilated.h"
...
int main(int argc, char** argv) {
    ...
    std::unique_ptr<Vconfigurable_brent_kung_adder> adder = std::make_unique<Vconfigurable_brent_kung_adder>();
    
    // Initialize VCD trace file
    Verilated::traceEverOn(true);
    std::unique_ptr<VerilatedVcdC> tfp = std::make_unique<VerilatedVcdC>();
    adder->trace(tfp.get(), 99);  // Trace 99 levels of hierarchy
    tfp->open("configurable_brent_kung_adder_sim.vcd");
