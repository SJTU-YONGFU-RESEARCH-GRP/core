#include "Vconfigurable_kogge_stone_adder.h"
#include "verilated.h"
...
void check_operation(std::unique_ptr<Vconfigurable_kogge_stone_adder>& adder, VerilatedVcdC* tfp, vluint64_t& sim_time) {
    ...
    std::cout << "Testing Configurable Kogge-Stone Adder with DATA_WIDTH=" << DATA_WIDTH << std::endl;
    std::cout << "------------------------------------------------------" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::unique_ptr<Vconfigurable_kogge_stone_adder> adder = std::make_unique<Vconfigurable_kogge_stone_adder>();
    
    // Initialize VCD trace file
    Verilated::traceEverOn(true);
    std::unique_ptr<VerilatedVcdC> tfp = std::make_unique<VerilatedVcdC>();
    adder->trace(tfp.get(), 99);  // Trace 99 levels of hierarchy
    tfp->open("configurable_kogge_stone_adder_sim.vcd");
