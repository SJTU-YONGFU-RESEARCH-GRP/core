...
void set_write_port_data(Vmulti_ported_fifo *dut, int port, uint32_t data) {
    // Since Verilator models vectored signals as packed arrays,
    // we need to manually set each bit for the specific port
    for (int i = 0; i < DATA_WIDTH; i++) {
        bool bit_value = (data >> i) & 1;
        dut->wr_data[port * DATA_WIDTH + i] = bit_value;
    }
}
...
uint32_t get_read_port_data(Vmulti_ported_fifo *dut, int port) {
    uint32_t data = 0;
    for (int i = 0; i < DATA_WIDTH; i++) {
        if (dut->rd_data[port * DATA_WIDTH + i]) {
            data |= (1 << i);
        }
    }
    return data;
...
void write_data(Vmulti_ported_fifo *dut, VerilatedVcdC *m_trace, 
    ...
    // Set write enables
    dut->wr_en[0] = write_port0;
    dut->wr_en[1] = write_port1;
...
void read_data(Vmulti_ported_fifo *dut, VerilatedVcdC *m_trace,
    ...
    // Set read enables
    dut->rd_en[0] = read_port0;
    dut->rd_en[1] = read_port1;
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < test_data.size(); i++) {
        ...
        std::cout << "Read data from port 0: 0x" << std::hex << data0 << std::dec
    ...
    std::cout << "FIFO full status: full=" << (int)dut->full 
              << ", port 0 ready=" << (int)dut->wr_ready[0]
              << ", port 1 ready=" << (int)dut->wr_ready[1] << std::endl;
    ...
    std::cout << "FIFO empty status: empty=" << (int)dut->empty 
              << ", port 0 valid=" << (int)dut->rd_valid[0]
              << ", port 1 valid=" << (int)dut->rd_valid[1] << std::endl;
    
    std::cout << "\nMulti-Ported FIFO test completed!" << std::endl;
