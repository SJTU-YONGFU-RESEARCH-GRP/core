...
// Fixing the Segmentation Fault in tb_configurable_param_fifo.cpp

Looking at the error message in the terminal, I can see that the program is crashing with a segmentation fault when testing the medium FIFO configuration (width=16, depth=8). This typically happens when the program tries to access memory it doesn't have permission to access.

After examining the code in `tb_configurable_param_fifo.cpp`, I've identified the issue. The problem is in the data masking logic. When working with a 16-bit data width, the mask calculation is incorrect and could lead to accessing invalid memory.
